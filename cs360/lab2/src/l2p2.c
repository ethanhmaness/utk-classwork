...
Machine *ReadMachine(int f)
{
    ...
    if (!read(f, ip, 4)) return free(ip), free(names), NULL;

    if (!read(f, buffer, 4)) return free(ip), free(names), NULL;
    ...
    for (int i = 0; i < num_names; i++)
    {
        ...
        do
        {
            if (!read(f, &c, 1)) return free(ip), free(names), NULL;
            strcat(name, &c);
        } while (c != '\0');
