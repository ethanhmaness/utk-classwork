...
class BasePolicy:
  ...
  def act(self, observations, return_log_prob=False):
    ...
    distribution: ptd.Distribution = self.action_distribution(observations)
    sample_tensor = distribution.sample()
    sampled_actions = sample_tensor.numpy()
    if return_log_prob:
      log_probs = distribution.log_prob(sample_tensor)
...
class GaussianPolicy(BasePolicy, nn.Module):
  ...
  def action_distribution(self, observations):
    ...
    distribution = ptd.Independent(ptd.Normal(self.network(observations), self.std()), 1)
